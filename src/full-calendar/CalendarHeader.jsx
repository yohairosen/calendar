    ...
    render() {
        ...
        const typeSwitcher = showTypeSwitch ? (
            ...
            <span className={switchCls}>
                ...
                {type === 'day' ?
                    <span className={`${switchCls}-focus`}>{locale.day || 'Day'}</span> :
                    <span
                        onClick={this.changeTypeToDay.bind(this)}
                        className={`${switchCls}-normal`}>{locale.day || 'Day'}</span>}


                {type === 'week' ?
                    <span className={`${switchCls}-focus`}>{locale.week || 'Week'}</span> :
                    <span
                        onClick={this.changeTypeToWeek.bind(this)}
                        className={`${switchCls}-normal`}>{locale.week || 'Week'}</span>}

                {type === 'date' ?
                    <span className={`${switchCls}-focus`}>{locale.month}</span> :
                    <span
                        onClick={this.changeTypeToDate.bind(this)}
                        className={`${switchCls}-normal`}>{locale.month}</span>
                }
                {type === 'month' ?
                    <span className={`${switchCls}-focus`}>{locale.year}</span> :
                    <span
                        onClick={this.changeTypeToMonth.bind(this)}
                        className={`${switchCls}-normal`}>{locale.year}</span>}
